...
template = """import { Card, CardContent, CardHeader } from "@/components/ui/card";
      ...
      <Card className="border-white/10 bg-white/5 backdrop-blur-md shadow-xl overflow-hidden">
        ...
        <CardContent className="p-0">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-white/5 bg-black/10">
            <span className="text-sm text-slate-400">Showing 1 to %%LEN%% of %%LEN%% entries</span>
...
def generate():
    ...
    for route, config in pages.items():
        ...
        comp_name = route.capitalize() + "Page"
        
        columns_list = "\n                  ".join([f'<th className="px-6 py-4 font-semibold">{c}</th>' for c in config["columns"]])
        
        row_renders = []
        for idx, col in enumerate(config["columns"]):
            if "Status" in col:
                row_renders.append(f'<td className="px-6 py-4"><span className={{`px-2.5 py-1 rounded-full text-xs font-medium ${{row["{col}"] === "Active" || row["{col}"] === "Completed" || row["{col}"] === "Approved" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/20" : row["{col}"] === "Pending" || row["{col}"] === "In Progress" || row["{col}"] === "Maintenance" || row["{col}"] === "Open" ? "bg-amber-500/20 text-amber-400 border border-amber-500/20" : "bg-red-500/20 text-red-400 border border-red-500/20"}}`}}>{{row["{col}"]}}</span></td>')
            elif idx == 0:
                row_renders.append(f'<td className="px-6 py-4 font-medium text-white group-hover:text-amber-400 transition-colors">{{row["{col}"]}}</td>')
            else:
                row_renders.append(f'<td className="px-6 py-4 text-slate-300">{{row["{col}"]}}</td>')
        
        row_renders_str = "\n                    ".join(row_renders)
        
        content = template.replace("%%COMP_NAME%%", comp_name)
        content = content.replace("%%DATA_JSON%%", json.dumps(config["data"], indent=4))
        content = content.replace("%%TITLE%%", config["title"])
        content = content.replace("%%DESC%%", config["description"])
        content = content.replace("%%COLUMNS%%", columns_list)
        content = content.replace("%%ROWS%%", row_renders_str)
        content = content.replace("%%LEN%%", str(len(config["data"])))
        
        with open(os.path.join(dir_path, "page.tsx"), "w", encoding="utf-8") as f:
            f.write(content)
            
    print("All 10 dummy pages generated successfully.")

...
import { Search, Filter, MoreVertical, ArrowDownToLine, CheckCircle2, XCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Deposits</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Monitor and manage user deposits.</p>
        </div>
                  ...
                  <tr key={i} className="hover:bg-white/80 dark:bg-[#111111]/80 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium flex items-center w-max gap-1 ${
                        ...
                        {row["Status"] === "Pending" && <ArrowDownToLine className="w-3 h-3" />}

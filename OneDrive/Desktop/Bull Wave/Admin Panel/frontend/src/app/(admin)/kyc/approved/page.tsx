...
import { Search, Filter, Clock, Eye, CheckCircle2, X, Loader2 } from "lucide-react";
...
export default function ApprovedKycPage() {
  ...
  const fetchKyc = async () => {
    try {
      setLoading(true);
      const res = await api.get("/admin/kyc/approved");
      setData(res.data);
    } catch (error) {
      console.error("Failed to fetch approved KYC", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Approved KYC</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Archive of all successfully verified users.</p>
        </div>
        ...
        <CardHeader className="border-b border-slate-100 dark:border-white/5 pb-4 bg-slate-50/50 dark:bg-white/5 flex flex-row items-center justify-between">
          <div className="relative w-full max-w-sm">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all placeholder:text-slate-500 dark:text-slate-400"
            />
          </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5 relative">
                {loading ? (
                  ...
                  filteredData.map((row) => (
                    <tr key={row.id} className="hover:bg-emerald-50/50 dark:hover:bg-white/5 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-slate-700 dark:text-slate-300 font-semibold">
                        {row.reviewed_by ? `Admin ID: ${row.reviewed_by}` : 'Auto-Approved'}
                      </td>
                      <td className="px-6 py-4 text-slate-700 dark:text-slate-300">

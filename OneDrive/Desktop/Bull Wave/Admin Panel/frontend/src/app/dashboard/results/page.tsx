  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-white">Result Management</h2>
          <p className="text-sm text-slate-400 mt-1">Declare game results and manage payouts.</p>
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredData.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-4 font-medium text-white group-hover:text-amber-400 transition-colors">{row["Match ID"]}</td>\n                    <td className="px-6 py-4 text-slate-300">{row["Game"]}</td>\n                    <td className="px-6 py-4 text-slate-300">{row["Outcome"]}</td>\n                    <td className="px-6 py-4 text-slate-300">{row["Total Bets"]}</td>\n                    <td className="px-6 py-4 text-slate-300">{row["Payout"]}</td>\n                    <td className="px-6 py-4 text-slate-300">{row["Declared At"]}</td>

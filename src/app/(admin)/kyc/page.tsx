...
export default function KycPage() {
  ...
  const data = [
    ...
    },
    {
        "User": "Priya Sharma",
        "Document Type": "Aadhaar Card",
        "Document Number": "XXXX-XXXX-9876",
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">KYC Management</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Review and approve user KYC submissions.</p>
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredData.map((row, i) => (
                  <tr key={i} className="hover:bg-white/80 dark:bg-[#111111]/80 transition-colors group">
                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-white group-hover:text-amber-400 transition-colors">{row["User"]}</td>
                    <td className="px-6 py-4 text-slate-700 dark:text-slate-300">{row["Document Type"]}</td>
                    <td className="px-6 py-4 text-slate-700 dark:text-slate-300">{row["Document Number"]}</td>
                    <td className="px-6 py-4 text-slate-700 dark:text-slate-300">{row["Submitted"]}</td>

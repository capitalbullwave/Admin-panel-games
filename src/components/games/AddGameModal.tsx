...
import { Upload, X, Check, Loader2, Plus, Image as ImageIcon, FileImage, Settings2, Info, CheckCircle2 } from "lucide-react";
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-4xl w-[95vw] bg-white dark:bg-[#111111] border-slate-200 dark:border-white/10 p-0 overflow-hidden shadow-2xl rounded-2xl max-h-[90vh] flex flex-col">
        <DialogHeader className="px-8 py-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 flex flex-row items-center justify-between shrink-0">
          <div>
            <DialogTitle className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Add New Game</DialogTitle>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Configure a new game for the platform.</p>
          </div>
        </DialogHeader>

        <div className="overflow-y-auto px-8 py-6 custom-scrollbar">
          <form id="add-game-form" onSubmit={handleSubmit(onSubmit as any)} className="space-y-10">
        ...
        <div className="border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-[#0a0a0a] px-8 py-4 shrink-0 flex items-center justify-end gap-3 rounded-b-2xl">
          ...
          <button 
            type="submit" 
            form="add-game-form"
            disabled={createGameMutation.isPending || uploadMutation.isPending}
          ...
          >
            {(createGameMutation.isPending || uploadMutation.isPending) ? (
